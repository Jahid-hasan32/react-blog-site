...
const ListOfArticle = () => {
  ...
  useEffect(() => {
    const fetchDataAuthor = async () => {
      try {
        ...
        console.error("Error fatching tags data:", error);
      }
    }
    fetchDataAuthor();
  }, [])

  const filteredWriter = tags_data.filter(item => item.tags.includes(tag)) 
  ...
  return (
    <div className='container mt-20 max-w-7xl '>
      <h1 className='text-center mb-8 underline underline-offset-4 font-bold text-2xl'>{tag} কতৃক লিখিত সমস্ত আর্টিকেল লিঙ্ক</h1>
